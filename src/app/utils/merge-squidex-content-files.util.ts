...
export async function mergeSquidexContentFiles(mergeFromFilePath: string, mergeIntoFilePath: string): Promise<void> {
    ...
    const patch = createTwoFilesPatch(mergeIntoFilePath, mergeFromFilePath, JSON.stringify(content2Json.data), JSON.stringify(content1Json.data));
    ...
    if(merged){
        ...
        console.log(`Merged diff from ${mergeFromFilePath} into ${mergeIntoFilePath}, updating ${mergeIntoFilePath} to match ${mergeIntoFilePath}`);
    }
    else{
        console.error(`Failed to merge ${mergeFromFilePath} into ${mergeIntoFilePath}. The files may not be compatible.`);
